...
/**
 * Etapa 126 — mesma lógica de lib/agenda/consulta.ts (buscarItensDoDia),
 * só que rodando no navegador em vez do servidor. É a peça que faz a
 * tela "Hoje" não depender mais de uma renderização no servidor pra
 * existir — o componente que chama isso decide o que fazer se der
 * erro (offline de verdade): mostra o que tiver em cache local.
 */
export async function buscarItensDoDiaCliente(
  ...
  const todosParticipantesUnicos = Array.from(new Set(Array.from(participantesPorHabito.values()).flat()));
...
export async function buscarCategoriasProdutividadeCliente(): Promise<{ id: string; nome: string; cor: string }[]> {
  const supabase = createClient();
  const { data } = await supabase.from("categorias_produtividade").select("id, nome, cor").order("nome");
  return data ?? [];
}
